...
(function() {
  var compile2SassAndJade, exec, fs, hassDir, iconv, jadeDir, log, pare, parse, path, publicDir, readFile, ref, sassDir, writeFile;
  ...
  ref = require('cirru-parser'), parse = ref.parse, pare = ref.pare;
  ...
  readFile = function(hass_file, sass_file, jade_file) {
    return fs.readFile(hass_file, function(err, data) {
      var simplifiedTree, str, syntaxTree;
      if (err) {
        log('2 读取文件fail' + err);
      } else {
      ...
      return syntaxTree.forEach(function(index, value) {
        log(index);
        return log(value);
      });
    });
